...
void DebugPrinter::Init()
{
	...
	if (image.Load("resources/font.bmp") != false)
	{
		...
		debugFont = SDL_CreateTexture(Renderer::Get()->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, target.GetWidth(), target.GetHeight());
...
void DebugPrinter::Print(int x, int y, int colour, const char* fmt, ...)
{
	char buffer[4096];
	...
	for (unsigned int i = 0; i < strlen(buffer); i++)
	{
		...
		default:
			...
			SDL_RenderCopy(Renderer::Get()->renderer, debugFont, &src, &dest);
